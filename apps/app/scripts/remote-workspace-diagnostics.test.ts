...
describe("resolveRemoteWorkspaceConnectionTarget", () => {
  test("builds a host-scoped Rantai target from saved worker credentials", () => {
  ...
  test("does not run Rantai probes against non-Rantai remote workspaces", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        openworkHostUrl: "",
    ...
    if (target.ok) return;
    expect(target.state.status).toBe("error");
    expect(target.state.message).toContain("Rantai remote workers");
  });

  test("does not run Rantai probes against stale Rantai fields on non-Rantai remotes", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        openworkHostUrl: "https://worker.example.com/w/ws_remote",
...
describe("testRemoteWorkspaceConnection", () => {
  ...
  test("reports a missing token after proving the worker endpoint is reachable", async () => {
    ...
    expect(result.state.message).toContain("Upgrade the Rantai host");
  ...
  test("uses fallback Rantai tokens saved on older workspace records", async () => {

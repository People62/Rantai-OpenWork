...
export function useDebugViewModel(options: UseDebugViewModelOptions) {
  ...
  const onRevealElectronMigrationBackup = useCallback(async () => {
    ...
    try {
      ...
      if (!appBundlePath) {
        setElectronMigrationStatus("Could not resolve the current Rantai.app bundle path.");
        return;
      }
      await revealDesktopItemInDir(`${appBundlePath}.migrate-bak`);
      setElectronMigrationStatus("Requested Finder reveal for Rantai.app.migrate-bak. The backup exists after an install handoff completes.");
    } catch (error) {
      setElectronMigrationStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, []);
  ...
  const onRestartOpenworkServer = useCallback(async () => {
    ...
    try {
      ...
      setOpenworkServiceStatus({
        tone: "success",
        message: t("settings.restart_succeeded_template", { service: "Rantai server" }),
      });
      ...
      setOpenworkServiceStatus({
        tone: "error",
        message: `${t("settings.restart_failed_template", { service: "Rantai server" })} ${message}`,
      });
      setServiceRestartError(message);
    } finally {
  ...
  const onCopyOpenworkLogs = useCallback(async () => {
    ...
    try {
      await navigator.clipboard.writeText(text);
      setOpenworkLogStatus(t("settings.copied_service_logs", { service: "Rantai server" }));
    } catch (error) {
  ...
  const onOpenResetModal = useCallback(
    (mode: ResetModalMode) => {
      ...
      const message =
        mode === "all"
          ? "Reset ALL Rantai app data? Open sessions and workspaces will be removed."
      ...
      void resetOpenworkState(mode)
        .then(async () => {
          ...
          setResetStatus(
            mode === "all"
              ? "Reset Rantai state. Restart the app to see changes."

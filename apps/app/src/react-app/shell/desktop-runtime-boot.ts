 ...
 *   2) if a local workspace is selected, restart the embedded Rantai server
 *   3) start the OpenCode engine pointed at the workspace
 *   4) activate the workspace on the running Rantai server
 ...
 *      localhost ports/tokens into Rantai settings.
 *
    ...
    void (async () => {
      try {
        ...
        const startServerWithoutDesktopWorkspace = async () => {
          setPhase("starting-engine", "Starting Rantai server");
          ...
          if (!isOpenworkServerInfoLike(serverInfo) || !isOpenworkServerReady(serverInfo)) {
            setError("Rantai server did not finish starting. Please restart Rantai.");
            return;
          }
        ...
        if (isElectronRuntime()) {
          ...
          if (boot.ok === false) {
            setError(boot.error || "Failed to start Rantai runtime");
            return;
          }

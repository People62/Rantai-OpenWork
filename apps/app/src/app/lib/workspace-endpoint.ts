 ...
 *
 * Every workspace-scoped API call in the app must route to the Rantai server
 * that actually owns that workspace. For local workspaces that's the user's
 * local Rantai server. For workspaces hosted on a remote Rantai worker
...
export type ResolvedWorkspaceEndpoint = {
  /** Host URL of the Rantai server that owns this workspace (no `/workspace` mount). */
  baseUrl: string;
  ...
  workspaceId: string;
  /** True when the workspace lives on a remote Rantai worker, not the user's local server. */
  isRemote: boolean;
